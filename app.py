...
# Mobile-first, máscaras em tempo real, divisão preview,
# responsável para menores, peso atual, sem modalidade
# ================================
...
def generate_registration_jpg(row: dict, event_name: str, region: str, logo_file=None, width=1080, height=1350) -> bytes:
    """Cartão da inscrição (inclui EID e peso atual)."""
    from PIL import ImageDraw, ImageFont
    img = Image.new("RGB", (width, height), color=(255, 255, 255)); draw = ImageDraw.Draw(img)
    try:
...
def generate_membership_card(img_profile_path: str, membership_id: str, eid: str, age: int, name: str, belt: str, width=900, height=600) -> bytes:
    """Carteirinha em JPG: foto, Membership ID, EID, idade, nome, faixa e QR."""
    from PIL import ImageDraw, ImageFont
...
def screen_new_registration():
    # Máscaras live (fora de form para rerun em cada digitação)
    ...
    if "guardian_phone" in st.session_state: st.session_state["guardian_phone"] = format_phone_live(st.session_state["guardian_phone"])
    if "eid" in st.session_state: st.session_state["eid"] = eid_format_live(st.session_state["eid"])
    if "guardian_eid" in st.session_state: st.session_state["guardian_eid"] = eid_format_live(st.session_state["guardian_eid"])
