...
# Minimal mobile-first, menu em botões, EID, carteirinha (membership) em JPG
# ================================
...
# ---- Cartão de inscrição (já existia) ----
...
def eid_is_valid(masked: str) -> bool:
    m = re.fullmatch(r"784-\d{4}-\d{7}-\d", masked or "")
...
def weight_options(mod, gen): return PESOS_ADULT_MASC_GI if (mod=="Gi" and gen=="Masculino") else (PESOS_ADULT_FEM_GI if mod=="Gi" else PESOS_NOGI)
def belts_for(mod): return FAIXAS_GI if mod=="Gi" else FAIXAS_NOGI
...
if "eid" not in st.session_state: st.session_state["eid"]=""

def label(label_text: str, key: str, required: bool=False):
    err = key in st.session_state["errors"]; color = "#b91c1c" if err else "#111"; star = " *" if required else ""
...
def admin_view():
    ...
    for _, row in df.iterrows():
        ...
        with st.expander(header, expanded=False):
            meta = {
                "ID": row.get("id",""),
                "Nome": f"{row.get('first_name','')} {row.get('last_name','')}".strip(),
            ...
            # Download cartão inscrição (completo)
...
def render_edit_form(best_row):
    ...
    with st.form(f"edit_{best_row['id']}", clear_on_submit=False):
        ...
        with colB:
            ...
            modality   = st.selectbox("Modalidade", MODALIDADES, index=0 if best_row.get("modality","Gi")=="Gi" else 1)
            belt_opts  = belts_for(modality)
            ...
            weight_opts= weight_options(modality, gender)
            weight     = st.selectbox("Peso", weight_opts, index=weight_opts.index(best_row.get("weight_class", weight_opts[0])) if best_row.get("weight_class","") in weight_opts else 0)
            academy    = st.text_input("Academia", value=best_row.get("academy",""))
            coach      = st.text_input("Professor/Coach", value=best_row.get("coach",""))
            coach_phone= st.text_input("Telefone do Professor", value=best_row.get("coach_phone",""))
    ...
    if submitted_update:
        ...
        upd = {
            "first_name": first, "last_name": last, "full_name_en": translate_to_english(f"{first} {last}"),
            ...
            "coach_phone": coach_phone, "modality": modality, "belt": belt, "weight_class": weight,
        }
...
def screen_new_registration():
    ...
    if "phone" in st.session_state: st.session_state["phone"] = format_phone_live(st.session_state["phone"])
    if "coach_phone" in st.session_state: st.session_state["coach_phone"] = format_phone_live(st.session_state["coach_phone"])
    if "eid" in st.session_state: st.session_state["eid"] = eid_format_live(st.session_state["eid"])
    ...
    with st.form("registration_form", clear_on_submit=False):
        ...
        colA, colB = st.columns(2)
        with colA:
            first_name_raw = st.text_input("Nome*", key="first_name", placeholder="Ex.: João")
            last_name_raw  = st.text_input("Sobrenome*", key="last_name", placeholder="Ex.: Silva")
        ...
        with colB:
            ...
            phone_input    = st.text_input("Telefone/WhatsApp (05_-___-____)*", key="phone", placeholder="052-123-4567")
            eid_mask       = st.text_input("EID (784-####-#######-#)*", key="eid", placeholder="784-1234-1234567-1")

            st.write("Data de nascimento*")
            c1, c2, c3 = st.columns([1,1,2])
            with c1:
                dob_day   = st.number_input("Dia", key="dob_day", min_value=1, max_value=31, step=1, value=st.session_state.get("dob_day", None))
            with c2:
                dob_month = st.number_input("Mês", key="dob_month", min_value=1, max_value=12, step=1, value=st.session_state.get("dob_month", None))
            with c3:
                current_year = dt.date.today().year
                dob_year  = st.number_input("Ano", key="dob_year", min_value=1900, max_value=current_year, step=1, value=st.session_state.get("dob_year", None))
        ...
        coach_phone_input = st.text_input("Telefone do Professor (05_-___-____)", key="coach_phone", placeholder="052-123-4567")

        # --- FOTOS ---
        st.subheader("Fotos obrigatórias")
        profile_img = st.camera_input("Foto de Perfil (rosto visível)*", key="profile_img")
        id_doc_img  = st.camera_input("Documento de Identificação (frente)*", key="id_doc_img")

        # Prévia da carteirinha (se já tiver foto e dados mínimos)
        try:
            have_min = profile_img is not None and st.session_state.get("belt") and st.session_state.get("first_name") and st.session_state.get("last_name")
            if have_min:
                # salva preview temporária da foto em memória para gerar prévia (sem ID definitivo)
                temp_reg = "PREVIEW"
                # idade baseada no ano (se já informado)
                age_preview = 0
                if st.session_state.get("dob_year"):
                    try: age_preview = compute_age_year_based(int(st.session_state["dob_year"]))
                    except Exception: pass
                name_preview = f"{title_capitalize(st.session_state['first_name'])} {title_capitalize(st.session_state['last_name'])}".strip()
                # salva a foto em arquivo temporário
                tmp_id = "TMP"
                tmp_path = save_image(profile_img, tmp_id, "profile_preview")
                pre_card = generate_membership_card(tmp_path, temp_reg, st.session_state.get("eid",""), age_preview, name_preview, st.session_state.get("belt",""))
                st.image(pre_card, caption="Prévia da carteirinha (Membership será gerado após envio)", use_column_width=True)
        except Exception:
            pass

        # --- COMPETIÇÃO ---
        st.subheader("Informações de Competição")
        modality = st.selectbox("Modalidade*", MODALIDADES, key="modality")
        belt     = st.selectbox("Faixa*", belts_for(modality), key="belt")
        weight_class = st.selectbox("Peso*", weight_options(modality, gender), key="weight_class")
    ...
    if submitted:
        errors = set()
        must = ["first_name","last_name","email","phone","gender","nationality","modality","belt","weight_class","consent","dob_day","dob_month","dob_year","profile_img","id_doc_img","academy_choice","eid"]
        for k in must:
        ...
        if st.session_state.get("eid") and not eid_is_valid(st.session_state["eid"]):
            errors.add("eid"); st.error("EID inválido. Use o formato 784-####-#######-#.")
        # Rosto
        ...
        if errors:
            st.error("Há campos obrigatórios faltando/invalidos. Corrija os destaques em vermelho e reenvie.")
            return
        ...
        try:
            ...
            st.error("Data de nascimento inválida."); return
        ...
        row = {
            ...
            "modality": st.session_state["modality"], "belt": st.session_state["belt"],
            "weight_class": st.session_state["weight_class"], "category": category,
            "consent": 1, "profile_photo_path": profile_photo_path, "id_doc_photo_path": id_doc_photo_path,
